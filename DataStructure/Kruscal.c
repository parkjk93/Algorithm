...
Vertex* SearchVertex(Graph* graph, char data) //있으면 true, 없으면 false
{
...
void Kruscal(Graph* graph, Graph* kruscal)
{
	Vertex* vertex = graph->vertices;
	Edge* edge = NULL;
	...
	while (vertex != NULL) //우선순위 큐에 그래프에 있는 모든 엣지를 넣는다.
	{
		edge = vertex->edgelist;
		if (edge != NULL)
		{
			while (edge != NULL)
			{
				EnQueue(queue, CreateNode(CreateEdge(edge->from, edge->target, edge->weight)));
				edge = edge->nextedge;
			}
		}
		vertex = vertex->nextvertex;
	...
	while (kruscal->index < graph->index)
	{
		Node* node = NULL;
		while (1)
		{
			...
			{
				if(!SearchEdge(kruscal, node->edge->from->data, node->edge->target->data))
					break;
			}
			else
				free(node);
		...
		Vertex* kruscalfrom = SearchVertex(kruscal, node->edge->from->data);
		Vertex* kruscaltarget = SearchVertex(kruscal, node->edge->target->data);
		
		if (kruscalfrom == NULL)
		{
			kruscalfrom = CreateVertex(node->edge->from->data);
			AddVertex(kruscal, kruscalfrom);
		}
		if (kruscaltarget == NULL)
		{
			kruscaltarget = CreateVertex(node->edge->target->data);
			AddVertex(kruscal, kruscaltarget);
		}
		
		AddEdge(kruscalfrom, kruscaltarget, node->edge->weight);
		AddEdge(kruscaltarget, kruscalfrom, node->edge->weight);
